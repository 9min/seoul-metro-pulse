...
│   └── renderLoop.ts           # 메인 렌더 루프 (requestAnimationFrame)
├── objects/
│   ├── StationNode.ts          # 역 노드 렌더링 객체
│   ├── LineLink.ts             # 노선 연결선 렌더링 객체
│   └── TrainPulse.ts           # 열차 입자 렌더링 객체
└── interactions/
    ├── zoomPan.ts              # 줌/팬 인터랙션
...
```ts
// src/services/trainPositionService.ts
const API_BASE_URL = import.meta.env.VITE_SEOUL_API_BASE_URL;
const API_KEY = import.meta.env.VITE_SEOUL_API_KEY;

export async function fetchTrainPositions(line: string) {
  const response = await fetch(
    `${API_BASE_URL}/${API_KEY}/json/realtimePosition/0/100/${line}`
  );
...
interface TrainState {
  ...
  selectTrain: (train: TrainPosition | null) => void;
}
